...
// Keep track of keys for escape sequence (Left Alt + Backslash)
...
void translate_and_send(struct input_event& ev, network::Client* client) {
    ...
    if (ev.type == EV_KEY) {
        ...
        if (ev.code == KEY_BACKSLASH) {
            backslash_pressed = (ev.value == 1 || ev.value == 2);
        }

        if (lalt_pressed && backslash_pressed) {
